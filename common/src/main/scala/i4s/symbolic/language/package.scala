  ...
  implicit class TreeSupport(sourceTree: Tree) {
    ...
    private def extractGrammar(tokenMap: Map[String,Token], parentTag: Option[GrammarTag]): Option[Grammar] =
      sourceTree match {
        case Terminal(token) =>
          parentTag match {
            case Some(posTag: PartOfSpeechTag) => tokenMap.get(token)
        ...
        case NonTerminal(tagString,children) =>
          PartOfSpeechTag.withNameOption(tagString).flatMap(pos => children.headOption.flatMap(tree => new TreeSupport(tree).extractGrammar(tokenMap,Some(pos)))) orElse
          PhraseTag.withNameOption(tagString).map(pos => Phrase(pos, children.toList.flatMap(tree => new TreeSupport(tree).extractGrammar(tokenMap,Some(pos))))) orElse
          ClauseTag.withNameOption(tagString).map(pos => Statement(pos, children.toList.flatMap(tree => new TreeSupport(tree).extractGrammar(tokenMap,Some(pos)))))
          .orElse {
            tagString match {
              case "ROOT" =>
