...
class CustomSortSpec extends AnyWordSpec with Matchers {
  ...
  "a CustomSort implmentation" should {
    ...
    "sort a list of users by age" in {
      ...
      // implement the sort
      implicit val order: Ordering[User] = Ordering.by[User, Int](_.age)
      val sorted = CustomSort.sort(users.toList)
    ...
    "sort a list of users by last name" in {
      ...
      // implement the sort
      implicit val order: Ordering[User] = Ordering.by[User, String](_.last)
      val sorted = CustomSort.sort(users.toList)
