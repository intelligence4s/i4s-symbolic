...
class RectMatable[T <: AnyVal] extends MappedMatable[Rect,T] {
  ...
  override def getMapped(mat: BaseMat[T], indices: Int*)(implicit indexer: Indexable[T], nl: NumberLike[T], tag: ClassTag[T]): Rect = {
    val ch = mat.channels
    val values = getN(mat, indices, ch).map(nl.toInt)
    Rect(values: _*)
  }

  override def getMappedN(mat: BaseMat[T], indices: Seq[Int], n: Int)(implicit indexer: Indexable[T], nl: NumberLike[T], tag: ClassTag[T]): IndexedSeq[Rect] = {
    val ch = mat.channels
    val values = getN(mat, indices, ch*n).map(nl.toInt)
    values.sliding(ch,ch).map(s => Rect(s:_*)).toIndexedSeq
  }

  override def putMapped(mat: BaseMat[T], indices: Seq[Int], value: Rect)(implicit indexer: Indexable[T], nl: NumberLike[T], tag: ClassTag[T]): Unit = {
    val ch = mat.channels
    val parts = value.asArray.take(ch).map(nl.fromInt)
    putN(mat,indices,parts)
  }

  override def putMappedN(mat: BaseMat[T], indices: Seq[Int], values: Seq[Rect])(implicit indexer: Indexable[T], nl: NumberLike[T], tag: ClassTag[T]): Unit = {
    val ch = mat.channels
